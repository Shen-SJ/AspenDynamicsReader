...
def read_data(variables):
    """讀取指定項目之歷史資料
    """
    # TODO: 更改資料格式，使用列表來分類所需資訊，如：{'Name': 'Moleflowrate',
    #                                              'Unit': 'kmole/hr',
    #                                              'Label': [UserDefine],
    #                                              'Data': [.........]}
    # 所有輸出的資料都會在這列表當中
    ...
    # 先建立時間序列
    time_list = []                                      # 所有時間都會在這列表當中
    variablep_record = variables[0][0].history             # 取得物流流量
    record_interval = variablep_record.Interval         # 取得紀錄區間
    record_starttime = variablep_record.StartTime       # 取得紀錄開始時間
    record_endtime = variablep_record.EndTime           # 取得紀錄結束時間

    time_list.append('Time')                            # 時間序列的標題:Time
    time_list.append('hr')                              # 時間序列的單位:hr
    for pre_time in range(int(record_starttime), int(record_endtime/record_interval+1)):
        time = pre_time*record_interval
        time_list.append(time)
        
#     print(len(time_list))      # 測試用:6001
    
    # 把時間序列加進數據列表當中
    data_set.append(time_list)
    ...
    for pre_obj in variables:                                     # pre_obj=(AspenObject, axial_label)
        ...
        var_list = []                                             # 目前該變數的所以資訊都會匯入到此列表，換個變數後即清空
        var_list.append(var_name + ', ' + label_name)             # var數據組的第一個資料:資料標題
        var_list.append(var_unit)                                 # var數據組的第二個資料：資料單位
        
        record_data = obj.History                                 # 所有的數據都在這history類別裡面
        # 用個迴圈把數據一個一個加到 var_list 當中
        for pre_time in range(int(record_starttime), int(record_endtime/record_interval+1)):
            time = pre_time*record_interval
            var_list.append(record_data.AtTime(time))
        
        # 如果數據資料長度跟時間長度不一致，需要檢查一下看出了什麼錯誤
        if len(var_list) != len(time_list):
            raise IndexError("The Data length is not equal to the time set, Pleas Check the Code !!!")
            
        # 數據長度與時間長度一致，就加入到 data_set 裡吧~
        data_set.append(var_list)
...
def plot_dynamic_results(data_set, 
    ...
    for i in range(1, num_plot+1):
        ...
        line = ax.plot(data_set[0][2:], data_set[i][2:], 'b-')     # 排除數據標題後的數據
        ax.set_xlim(0, data_set[0][-1])
        ax.set_xlabel('Time (hr)')
        ax.set_ylabel(f'{data_set[i][0]} ({data_set[i][1]})')
        user_plot_setting(i, ax_obj_list=ax_obj_list)            # 使用者自訂的一些圖形規格(ex: 座標標籤、座標scale......)
...
def multiplot_dynamic_results(data_set_group, 
    ...
    for i in range(1, num_plot+1):
        ...
        for num_plot in data_set_group:
            line = ax.plot(num_plot[0][2:], num_plot[i][2:])        # 排除數據標題後的數據
        line_group_list.append(ax.lines)
        
        ax.set_xlim(0, data_set_group[0][0][-1])
        ax.set_xlabel('Time (hr)')
        ax.set_ylabel(f'{data_set_group[0][i][0]} ({data_set_group[0][i][1]})')
...
def user_plot_setting(index, ax_obj_list):
...
#     """  
    ...
    multiplot_dynamic_results([data2, data3], 
